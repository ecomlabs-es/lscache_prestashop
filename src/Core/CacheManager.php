...
class CacheManager
{
    ...
    public function setCacheControlHeader(): void
    {
        ...
        //
        // We also strip Set-Cookie headers on cacheable responses for two
        // reasons: LSWS (depending on respCookieCache/cacheIgnoreSetCookie
        // config) can refuse to store the body when Set-Cookie is present,
        // and per-visitor cookies like PHPSESSID or PrestaShop-* must not
        // be baked into a shared cache entry. Visitor-specific session state
        // is regenerated by the private ESI sub-requests on each delivery.
        if (CacheState::isCacheable()) {
